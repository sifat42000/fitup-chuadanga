...
const Checkout = () => {
    ...
    const saveOrderToDB = async (orderData) => {
        try {
            const { data: saveOrderResponse } = await axios.post('/api/payment/save-order', orderData)
            if (!saveOrderResponse.success) {
                throw new Error(saveOrderResponse.message)
            }
            return { success: true, orderId: saveOrderResponse.data?.orderId || orderData.razorpay_order_id }
        } catch (error) {
    ...
    const placeOrder = async (formData) => {
        ...
        try {
            ...
            if (chosenPaymentMethod === 'cod') {
                ...
                showToast('success', 'Order placed successfully! You will pay at delivery.')
                dispatch(clearCart())
                orderForm.reset()
                ...
                const razOption = {
                    ...
                    "handler": async function (response) {
                        ...
                        try {
                            ...
                            showToast('error', error.message || 'Failed to save order')
                        } finally {
                            setSavingOrder(false)
                        }
                    },
